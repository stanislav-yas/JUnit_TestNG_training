...
public class CreateNewFile2Test extends TestBase implements MyCategories{
    ...
    @UseDataProvider(value = "charsProvider", location = MyDataProvider.class)
    @DataSource(value = "/allow_spec_chars", type = DataSource.Type.RESOURCE)
    public void allowSpecChars(String chr){
    ...
    @UseDataProvider(value = "charsProvider", location = MyDataProvider.class)
    @DataSource(value = "/not_allow_spec_chars", type = DataSource.Type.RESOURCE)
    public void notAllowSpecChars(String chr){

        StringBuffer buf = new StringBuffer(RndStrGen.getRndStr(RndStrGen.latinChars, 25));
        int rndOffset = RndStrGen.rng.nextInt(buf.length());
        String filename = buf.insert(rndOffset,chr).toString(); // insert spec char to random position of string
        try{
            boolean result = new File(MySuite.tmpDir.getRoot(), filename).createNewFile();
