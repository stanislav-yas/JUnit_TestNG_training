...
public class CreateNewFile1Test extends TestBase implements MyCategories{
    ...
    @UseDataProvider(value = "randomStringProvider", location = MyDataProvider.class)
    @DataSource(value = RndStrGen.latinChars + RndStrGen.numericChars,
            type = DataSource.Type.STRING, params = {5, 20})
    ...
    @Category(Positive.class)
    @UseDataProvider(value = "randomStringProvider", location = MyDataProvider.class)
    @DataSource(value = RndStrGen.latinChars + RndStrGen.cyrillicChars + RndStrGen.numericChars,
            type = DataSource.Type.STRING, params = {7, 40})
    public void latinCyrillicNumericFilename(String filename) throws IOException{
        File file = new File(MySuite.tmpDir.getRoot(), filename);
        boolean result = file.createNewFile();
        Assert.assertTrue("File has not created with name:" + filename, result && file.exists());
    }
